...
!
!write(*,'("Number of CMSI atoms ",i3," and COSMO atoms ",i3)')natom,n
n = natom
write(*,'("Number of CMSI atoms ",i3," and COSMO atoms ",i3)')natom,n
!write(*,'number of atoms cosmo(I3)')n
inquire(file="Input.txt", exist=exist)
if (exist) then
  open (unit=14,file="Input.txt",status="old",form="formatted",access="sequential")
else
  open(14, file="Input.txt", status="new", action="read")
endif
!
...
!
!read(14,*) iprint      ! printing flag
iprint = 3
!read(14,*) nproc
nproc= 16       ! number of openmp threads
!read(14,*) lmax        ! max angular momentum of spherical harmonics basis
lmax = 6
!read(14,*) ngrid       ! number of lebedev points
ngrid = 110
!read(14,*) iconv       ! 10^(-iconv) is the convergence threshold for the iterative solver
iconv = 8
!read(14,*) igrad       ! whether to compute (1) or not (0) forces
igrad = 1
!read(14,*) eps         ! dielectric constant of the solvent
eps= 78.3553
!read(14,*) eta         ! regularization parameter
eta = 0.1
!
!read(14,*) n           ! number of atoms
!
allocate (charge(n),x(n),y(n),z(n),rvdw(n))
!
...
!
do i = 1, n
  read(14,*) charge(i), x(i), y(i), z(i), rvdw(i)
enddo
...
write (6,'(1x,a,f14.6)') 'ddcosmo electrostatic solvation energy hartrees i think: ', esolv
cosmoe = esolv
escf = (escf + cosmoe) 
emp2 = (emp2 + cosmoe)
...
!  if (iprint.ge.3) write(*,'Number of atoms ',n)
!
