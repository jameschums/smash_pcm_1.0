...
&               "hope it never prints zero mp2 ",f14.7," hartrees escf+escsmp2",/)')jamesflag,(escf+escsmp2)
      if (escf /= 0) then 
        write(*,'("energies mp2 ",i3,2x,f14.7," hartrees escf")')jamesflag,(escf)
      endif
      if (emp2 /=0) then
        write(*,'("energies mp2 ",i3,2x,f14.7," hartrees emp2")')jamesflag,(emp2)
      endif 
      if (escsmp2 /=0) then 
        write(*,'("energies mp2 ",i3,2x,f14.7," hartrees escsmp2")')jamesflag,(escsmp2)
      end if
      ...
      if (jamesflag >= 1) then 
        write(13, '(I3)')natom
!       write(12, '("   ")')
        write(13, '(f14.7)')(escf+emp2)
        do i= 1,natom
          write(13,'(3x,a3,3x,3f14.7)')table(numatomic(i)),(coord(j,i)*toang,j=1,3)
        end do
      else
        write(*,'("jamesfirst,jamesflag,jamese ",a9,2x,i3,2x,3f3.6)')jamesfirst,jamesflag,jamese
      endif
      close(13)
